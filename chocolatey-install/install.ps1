...
if (![string]::IsNullOrEmpty($chocolateyVersion)){
  ...
  $url = "https://artifactorydk.3shape.local/chocolatey/chocolatey.$chocolateyVersion.nupkg"
}
...
if ($url -eq $null -or $url -eq '') {
  ...
  $url = 'https://artifactorydk.3shape.local/api/nuget/chocolatey/Packages()?$filter=((Id%20eq%20%27chocolatey%27)%20and%20(not%20IsPrerelease))%20and%20IsLatestVersion'
...
if ($useWindowsCompression -ne $null -and $useWindowsCompression -eq 'true') {
  ...
  # download 7zip
  Download-File 'https://artifactorydk.3shape.local/Download/tools/7za.exe' "$7zaExe"
}
...
# remove chocolatey source
choco sources remove -name chocolatey

# add threeshape chocolatey source
choco sources add -source https://artifactorydk.3shape.local/api/nuget/chocolatey-local -name threeshape -priority 1
choco sources add -source https://artifactorydk.3shape.local/api/nuget/chocolatey-remote -name chocolatey -priority 2

# using two sources since https://www.jfrog.com/jira/browse/RTFACT-17889
